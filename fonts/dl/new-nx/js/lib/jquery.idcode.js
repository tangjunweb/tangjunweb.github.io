...
(function($){
	...
	$.idcode = {
		getCode:function(option){
		    _commSetting(option, settings);
			return _storeData(_set.storeLable, null);
		},
		setCode:function(option){
		    _commSetting(option, settings);
		    _setCodeStyle("#" + settings.e, settings.codeType.name, settings.codeType.len, settings);
	...
	$.idcode1 = {
	    getCode: function (option) {
	        _commSetting(option, settings1);
	        return _storeData(_set.storeLable, null);
	    },
	    setCode: function (option) {
	        _commSetting(option, settings1);
	        _setCodeStyle("#" + settings1.e, settings1.codeType.name, settings1.codeType.len, settings1);

	    },
	    validateCode: function (option) {
	        _commSetting(option, settings1);
	        var inputV;
	        if (settings1.inputID) {
	            inputV = $('#' + settings1.inputID).val();
	        } else {
	            inputV = $(_set.store).val();
	        }
	...
	$.idcode2 = {
	    getCode: function (option) {
	        _commSetting(option, settings2);
	        return _storeData(_set.storeLable, null);
	    },
	    setCode: function (option) {
	        _commSetting(option, settings2);
	        _setCodeStyle("#" + settings2.e, settings2.codeType.name, settings2.codeType.len, settings2);
	...
	$.idcode3 = {
	    getCode: function (option) {
	        _commSetting(option, settings3);
	        return _storeData(_set.storeLable, null);
	    },
	    setCode: function (option) {
	        _commSetting(option, settings3);
	        _setCodeStyle("#" + settings3.e, settings3.codeType.name, settings3.codeType.len, settings3);
	...
	function _commSetting(option,setItem){
	    $.extend(setItem, option);
	}
	...
	function _setCodeStyle(eid, codeType, codeLength, setItem) {
	    var codeObj = _createCode(setItem.codeType.name, setItem.codeType.len);
